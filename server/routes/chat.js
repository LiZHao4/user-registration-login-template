...
router.get('/chat/:id', authMiddleware, async (req, res) => {
  try {
    ...
    const validateResult = await db.getOne(`
      SELECT
        (SELECT COUNT(*) FROM friendships WHERE id = ? AND (source = ? OR target = ?)) +
        (SELECT COUNT(*) FROM group_members WHERE \`group\` = ? AND user = ?)
        AS validate
    ...
    let type = null
    const friendshipCheck = await db.getOne(
      'SELECT id FROM friendships WHERE id = ? AND (source = ? OR target = ?)',
    ...
    if (friendshipCheck) {
      ...
      const groupMemberCheck = await db.getOne(
        'SELECT 1 FROM group_members WHERE `group` = ? AND user = ?',
        [sessionId, userId]
      )
      if (groupMemberCheck) {
        type = 'group'
    ...
    let chatQuery = `
      SELECT id, content, UNIX_TIMESTAMP(sent_at) AS sent_at, sender, multi, type 
      FROM chats 
      WHERE session = ? `
    let queryParams = []
    let needReverse = false
    if (parsedMin !== null && parsedMax !== null) {
      chatQuery += 'AND id >= ? AND id <= ? ORDER BY id ASC'
      queryParams = [sessionId, parsedMin, parsedMax]
    } else if (parsedMax !== null) {
      chatQuery += 'AND id > ? ORDER BY id ASC'
      queryParams = [sessionId, parsedMax]
      if (parsedNum !== null) {
        chatQuery += ` LIMIT ${parsedNum}`
      }
    } else if (parsedMin !== null) {
      chatQuery += 'AND id < ? ORDER BY id DESC'
      ...
      chatQuery += `ORDER BY id DESC LIMIT ${parsedNum}`
      queryParams = [sessionId]
      needReverse = true
    } else {
      chatQuery += 'ORDER BY id'
      queryParams = [sessionId]
    }
      ...
      if ([3, 4, 5, 6].includes(chat.type)) {
        try {
          ...
          if (chat.type === 4 && chat.content && chat.content.target) {
            const innerUser = await db.getOne('SELECT nick FROM users WHERE id = ?', [chat.content.target])
            if (innerUser) {
          ...
          if (chat.type === 6 && chat.content && Array.isArray(chat.content)) {
            ...
            if (userIds.size > 0) {
              ...
              const userInfos = await db.query(
                `SELECT id, nick, user_avatar FROM users WHERE id IN (${placeholders})`,
                userIdArray
              )
              const userInfoMap = {}
      ...
      switch (type) {
        case 'friend':
          ...
          const friendship = await db.getOne(`
            SELECT
              CASE WHEN source = ? THEN target ELSE source END AS other_user_id,
              UNIX_TIMESTAMP(request_time) AS request_time,
              UNIX_TIMESTAMP(allowed_time) AS allowed_time
            FROM friendships
            WHERE id = ?
          `, [userId, sessionId])
          if (friendship && friendship.other_user_id) {
            const otherUser = await db.getOne(
              'SELECT nick, user_avatar FROM users WHERE id = ?',
              [friendship.other_user_id]
            )
            const remark = await db.getOne(
              'SELECT remark FROM user_remarks WHERE user_id = ? AND target_user_id = ?',
              [userId, friendship.other_user_id]
            )
            metaData = {
              id: userId,
        ...
        case 'group':
          const groupMembers = await db.query(
            'SELECT user, role, group_nickname FROM group_members WHERE `group` = ? ORDER BY joined_at',
            [sessionId]
          )
          ...
          const joinedAt = await db.getOne(
            'SELECT UNIX_TIMESTAMP(joined_at) AS joined_at FROM group_members WHERE `group` = ? AND user = ?',
            [sessionId, userId]
          )
          const groupInfo = await db.getOne(
            'SELECT group_name, group_info_permission, group_avatar FROM `groups` WHERE id = ?',
            [sessionId]
          )
          metaData = {
