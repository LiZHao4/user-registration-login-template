...
router.post('/send', authMiddleware, async (req, res) => {
  try {
    ...
    try {
      ...
      const sender = await db.getOne('SELECT nick, user_avatar FROM users WHERE id = ?', [userId])
      const senderNick = sender.nick
      const senderAvatar = sender.user_avatar
      if (friendship) {
        ...
        const unreadCount = await db.getOne('SELECT COUNT(*) AS count FROM chats WHERE session = ? AND sender != ? AND id > (SELECT COALESCE((SELECT max_id FROM message_read_status WHERE session_id = ? AND user_id = ?), 0))', [targetId, receiverId, targetId, receiverId])
        ...
        const groupAvatarRow = await db.getOne('SELECT group_name, group_avatar FROM `groups` WHERE id = ?', [targetId])
        ...
        for (const member of groupMembers) {
          if (member.user !== userId) {
            const remarkRow = await db.getOne('SELECT remark FROM user_remarks WHERE user_id = ? AND target_user_id = ?', [member.user, userId])
            const remark = remarkRow?.remark
            const unreadCount = await db.getOne('SELECT COUNT(*) AS count FROM chats WHERE session = ? AND sender != ? AND id > (SELECT COALESCE((SELECT max_id FROM message_read_status WHERE session_id = ? AND user_id = ?), 0))', [targetId, member.user, targetId, member.user])
            const unreadCountValue = unreadCount.count
            const pushData = {
              id: result.insertId,
              session: targetId,
              ...
              msg_nick: senderNick,
              remark,
              group_avatar: groupAvatarRow.group_avatar,
              members: groupMembers.length,
              group_name: groupAvatarRow.group_name,
