...
export async function getDisplayContent0(item: MessageType, currentUserId: number): Promise<string> {
  ...
  const data = {
    ...
    msg_sender: item.sender,
    msg_nick: item.msg_nick,
    unread_count: item.unread_count,
    type: item.type
  } as FriendItem
  if (isTypeFriend) {
    data['avatar'] = item.sender_avatar
    data['friend_id'] = item.sender
  } else if (isTypeGroup) {
    data['avatar'] = item.group_avatar
    data['member_count'] = item.members
  }
  if (item.msg_type === 4 && JSON.parse(data.content).target) {
    const target = JSON.parse(data.content).target
    const targetData = await axios.get('/api/user/' + target)
    data['inner_nick'] = targetData.data.nick
  }
  return getDisplayContent(data, currentUserId)
}